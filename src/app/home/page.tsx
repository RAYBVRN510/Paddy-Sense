...
export default function HomePage() {
  ...
  const stats = useMemo(() => {
    ...
    const highSeverity = scans.filter((scan) => scan.severity === "high").length;
    const healthy = scans.filter((scan) => scan.disease === "Healthy").length;
    return { total, highSeverity, healthy };
  }, [scans]);
    ...
    <div className="mx-auto max-w-5xl space-y-4">
      <section className="surface p-6 sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">Quick actions and your latest field insights in one place.</p>
          </div>
          <div className="grid w-full gap-2 sm:w-auto sm:grid-cols-2">
            <Link href="/camera" className="btn-primary w-full">
              Start Scan
            </Link>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <article className="surface p-5">
          ...
          <p className="mt-3 text-3xl font-semibold text-green-700">{stats.total}</p>
        ...
        <article className="surface p-5">
          ...
          <p className="mt-3 text-3xl font-semibold text-red-600">{stats.highSeverity}</p>
        ...
        <article className="surface p-5">
          ...
          <p className="mt-3 text-3xl font-semibold text-emerald-700">{stats.healthy}</p>
      ...
      <section className="grid gap-3 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="surface p-6 sm:p-8">
          ...
          {recentScans.length === 0 ? (
            <div className="mt-6 rounded-2xl border border-dashed border-slate-300 p-6 text-center">
              <p className="text-sm text-slate-600">No scans yet. Start a new scan to build your history.</p>
            ...
            <div className="mt-6 space-y-3">
              {recentScans.map((scan) => (
                <div key={scan.id} className="surface-soft flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
        ...
        <div className="space-y-3">
          <div className="surface p-6">
            ...
            {latestScan ? (
              <div className="mt-4 space-y-2 text-sm text-slate-600">
                <p>
                  <span className="font-semibold text-slate-800">Disease:</span> {latestScan.disease}
                </p>
                <p>
                  <span className="font-semibold text-slate-800">Confidence:</span> {latestScan.confidence}%
                </p>
                <p>
                  <span className="font-semibold text-slate-800">Severity:</span> {latestScan.severity}
                </p>
                <p>
                  <span className="font-semibold text-slate-800">Date:</span> {latestScan.date.toLocaleDateString()}
                </p>
              </div>
            ) : (
              <p className="mt-3 text-sm text-slate-600">No scans yet. Capture your first leaf to populate this panel.</p>
            )}
          </div>
          ...
          <div className="surface p-6">
            ...
            <Link href="/settings" className="mt-4 inline-flex text-sm font-semibold text-green-700 hover:text-green-800">
              Update preferences
            </Link>
          </div>
        </div>
